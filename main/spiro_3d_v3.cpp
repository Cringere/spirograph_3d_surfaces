...
int steps;
float step_delta;
//float point_r = 0.2f;		//minimum spacing between elements
...
public:
	...
	Triangle(Vertex *a, Vertex *b, Vertex *c) : v{a, b, c} {
		//a->remove_nearby_vertex(b);
		//a->remove_nearby_vertex(c);
		a->connected_triangles.push_back(this);

		//b->remove_nearby_vertex(a);
		//b->remove_nearby_vertex(c);
		b->connected_triangles.push_back(this);
		
		//c->remove_nearby_vertex(a);
		//c->remove_nearby_vertex(b);
	...
	void recursively_build() {
		...
		for (int i = 0; i < 3; ++i) {
			...
			for (int i = 0; i < a->nearby_vertices.size(); ++i) {
				Vertex *v = a->nearby_vertices[i];
				for (int j = 0; j < b->nearby_vertices.size(); ++j) {
					Vertex *w = b->nearby_vertices[j];
					if (v == w) {
						float d = (*v - line_center).angle_cos(line_normal);
						if (d > max_angle_cos) {

							Math::Vector3D new_normal = (*v - *a).cross(*b - *v).normalize();

							//check other triangles connected to this vertex
							for (Triangle *t1: v->connected_triangles) {
								for (Triangle *t2: a->connected_triangles) {
									for (Triangle *t3: b->connected_triangles) {
										if (t1 == t2 && t2 == t3) {
											if (new_normal.angle_cos(normal()) > 0.1f) {
												goto end;
											}
										}
									}
			...
			if (c != nullptr && max_angle_cos > 0.8f) {
				Triangle *triangle = new Triangle(a, c, b);
				spiro_surface_triangles.push_back(triangle);
				triangle->recursively_build();
			}
		}
	}
							...
							//for (Triangle *t1: v->connected_triangles) {
								for (Triangle *t2: a->connected_triangles) {
									for (Triangle *t3: b->connected_triangles) {
										//if (t1 == t2 || t1 == t3) {
										if (t2 == t3 && t2 != this) {
											if (new_normal.angle_cos(normal()) > 0.0f) {
												goto end;
											}
										}
									}
...
static void create_spirograph_surface() {
	...
	//create the surface
	create_surfrace();
	//s.init(spiro_surface_triangles.size() * 6, std::vector<int> {3, 3, 3});
	//s.clear();
	//std::vector<float> surface_data(spiro_surface_triangles.size() * 3 * (3 + 3 + 3));	//3 vectors per triangle, poisition, normal, color per vector
	s.init(10000 * 6, std::vector<int> {3, 3, 3});
	s.clear();
	std::vector<float> surface_data(10000 * 3 * (3 + 3 + 3));	//3 vectors per triangle, poisition, normal, color per vector
	Math::Vector3D color(0.1f, 0.7f, 0.3f);
	for (int i = 0, count = 0, m = spiro_surface_triangles.size(); i < m; ++i) {
	...
	s.add_data(&surface_data[0], surface_data.size());
	s.update();
}
